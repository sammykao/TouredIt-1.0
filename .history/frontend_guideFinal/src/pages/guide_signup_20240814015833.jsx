...
export function GuideSignUp() {
  ...
  const handleActivityChange = (index, e) => {
    const { name, value } = e.target;
    const updatedActivities = expData.map((activity, i) => {
      if (i === index) {
  ...
  const handleRemoveWorkExp = (index) => {
    const updatedExp = hobbyData.filter((_, i) => i !== index);
    setExpData(updatedExp);
  };

  const handleAddWorkExp = () => {
    setExpData([...activityData, { job_name: '', description: '' }]);
  };

  const handleWorkExpChange = (index, e) => {
    const { name, value } = e.target;
    const updatedExp = activityData.map((activity, i) => {
      if (i === index) {
        return { ...activity, [name]: value };
      }
      return activity;
