...
export function UpdateActivities() {
  ...
  const addActivity = async (e) => {
    e.preventDefault();
    try {
      const response = await fetch('https://lera2ibwne.execute-api.us-east-2.amazonaws.com/api/newActivity', {
        method: 'POST',
        headers: {
  ...
  const deleteActivity = async (id) => {
    if (window.confirm("Are you sure you want to delete this activity?")) {
      try {
        const response = await axios.post('https://lera2ibwne.execute-api.us-east-2.amazonaws.com/api/remInvolvement', { id });
        if (response.status === 200 || response.status === 201) {
          const updatedActivities = responseData.guide.activities.filter(activity => activity.activity_id !== id);
            ...
            <div className="flex flex-col items-center sm:flex-row sm:items-start">
              ...
              <div className="mt-4 sm:mt-0 sm:ml-4 text-center sm:text-left">
                ...
                <div className="mt-2 flex flex-col sm:flex-row sm:space-x-2">
                  ...
                  <Link to="/update-work-experience" className='bg-blue-500 hover:bg-blue-900 text-white font-bold py-1 px-3 rounded-md text-center mb-2 sm:mb-0'>
                    Update Work Experience
                  </Link>
                  <Link to="/tours" className='bg-blue-500 hover:bg-blue-900 text-white font-bold py-1 px-3 rounded-md text-center mb-2 sm:mb-0'>
                    Tours
                  </Link>
            ...
            <div className="mt-2 hidden sm:block">
              <Typography className="text-lg font-semibold text-gray-700 mb-4">
                <div className="flex py-2">
                  <span className="font-bold w-full sm:w-1/4">Activity</span>
                  <span className="font-bold w-full sm:w-1/4">Description</span>
                </div>
              </Typography>
            </div>
            <ul className="text-gray-700">
              {responseData.guide.activities[0]?.activity_name ? (
                  ...
                  <li key={key} className="border-y py-2">
                    <div className="flex flex-col sm:flex-row sm:items-center">
                      <div className="w-full sm:w-1/4 mb-2 sm:mb-0">
                        <span className="font-bold sm:hidden">Activity:</span>
                        <span className="text-gray-700 ml-1">{activity.activity_name}</span>
                      </div>
                      <div className="w-full sm:w-1/4 mb-2 sm:mb-0">
                        <span className="font-bold sm:hidden">Description:</span>
                        <span className="text-gray-700 ml-1 block truncate">{activity.description}</span>
                      </div>
