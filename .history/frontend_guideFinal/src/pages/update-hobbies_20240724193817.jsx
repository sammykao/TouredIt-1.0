...
export function UpdateHobbies() {
  ...
  const deleteHobby = async (hobbyId) => {
    if (window.confirm("Are you sure you want to delete this hobby?")) {
      try {
        const response = await axios.post('https://zytxastigf5jf3p5qhcb472ba40icqyo.lambda-url.us-east-2.on.aws/api/remHobby', { hobby_id: hobbyId });
        if (response.status === 201) {
          ...
          setResponseData(prevState => ({
            ...
            guide: {
              ...prevState.guide,
              hobbies: prevState.guide.hobbies.filter(hobby => hobby.hobby_id !== hobbyId)
            }
          }));
        }
      } catch (error) {
      ...
      <div className='min-h-screen'>
        ...
        <div className="bg-gray-100 p-4">
          <div className="max-w-7xl mx-auto bg-white rounded-lg shadow-md p-8">
            <div className="mt-2 hidden sm:block">
              <Typography className="text-lg font-semibold text-gray-700 mb-4">
                <div className="flex py-2">
                  <span className="font-bold w-full sm:w-1/4">Hobbies</span>
            ...
            <ul className="text-gray-700">
              {responseData.guide.hobbies.length > 0 ? (
                responseData.guide.hobbies.map((hobby, key) => (
                  <li key={key} className="border-y py-2">
                    <div className="flex flex-col sm:flex-row sm:items-center">
                      ...
                      <div className="w-full sm:w-1/4 mb-2 sm:mb-0">
                        <span className="font-bold sm:hidden">Description: {responseData.guide.hobbies.length }</span>
