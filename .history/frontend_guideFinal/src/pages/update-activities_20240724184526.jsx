...
export function UpdateActivities() {
  ...
  const deleteActivity = async (id) => {
    if (window.confirm("Are you sure you want to delete this activi"))
    try {
      ...
      if (response.status === 201) {
        ...
        setResponseData(prevState => ({
          ...
          guide: {
            ...prevState.guide,
            activities: prevState.guide.activities.filter(activity => activity.id !== id)
          }
        }));
      }
    } catch (error) {
              ...
              {responseData.guide.activities.map((activity, key) => (
                <li key={key} className="flex flex-col sm:flex-row items-center border-y py-2">
                  ...
                  <button
                    ...
                    className="bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded mt-2 sm:mt-0 sm:ml-4"
                  >
                    Delete
                  </button>
                </li>
