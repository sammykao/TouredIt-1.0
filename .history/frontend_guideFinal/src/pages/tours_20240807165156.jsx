...
export function Tours() {
  ...
  useEffect(() => {
    ...
    axios.post("https://lera2ibwne.execute-api.us-east-2.amazonaws.com/api/retTours", { email })
      .then(tourResponse => {
  ...
  const handleConfirm = async (id) => {
    try {
      const confirmBox = window.confirm(
        `Are you sure you want to CONFIRM this tour? If you accept, you will be EMAILED with next steps to follow.`
      );
      if (confirmBox) {
        await axios.post("https://lera2ibwne.execute-api.us-east-2.amazonaws.com/api/confirmTour", { id });
        const updatedPendingTours = pendingTours.filter(tour => tour.id !== id);
        const confirmedTour = pendingTours.find(tour => tour.id === id);
        setPendingTours(updatedPendingTours);
        setConfirmedTours([...confirmedTours, confirmedTour]);
      }
    } catch (error) {
      setError(error);
    }
  ...
  const handleCancel = async (id) => {
    try {
        await axios.post("https://lera2ibwne.execute-api.us-east-2.amazonaws.com/api/cancelTour", { id });
        const updatedConfirmedTours = confirmedTours.filter(tour => tour.id !== id);
        setConfirmedTours(updatedConfirmedTours);
    } catch (error) {
      setError(error);
    }
  };
  ...
  const handleReject = async (id) => {
    try {
      await axios.post("https://lera2ibwne.execute-api.us-east-2.amazonaws.com/api/declineTour", { id });
      const updatedPendingTours = pendingTours.filter(tour => tour.id !== id);
      setPendingTours(updatedPendingTours);
    } catch (error) {
      setError(error);
