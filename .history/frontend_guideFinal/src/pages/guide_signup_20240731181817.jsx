...
export function GuideSignUp() {
  ...
  const insertBackend = async () => {
    ...
    try {
      ...
      await axios.post('https://zytxastigf5jf3p5qhcb472ba40icqyo.lambda-url.us-east-2.on.aws/api/newGuide', postData);

      for (const hobby of hobbyData) {
        hobby.email = email;
        await axios.post('https://zytxastigf5jf3p5qhcb472ba40icqyo.lambda-url.us-east-2.on.aws/api/newHobby', hobby);
      }

      for (const activity of activityData) {
        activity.email = email;
        await axios.post('https://zytxastigf5jf3p5qhcb472ba40icqyo.lambda-url.us-east-2.on.aws/api/newActivity', activity);
      }
            ...
            {!isVerifying ? (
              <form onSubmit={handleSubmit} className="w-full max-w-lg ">
                ...
                <div>
                  {hobbyData.map((hobby, index) => (
                    <div key={index} className="flex flex-wrap -mx-3 mb-6">
                      ...
                      <div className="w-full flex justify-end mt-2">
                        <Button color="red" onClick={() => handleRemoveHobby(index)} className="ml-2">
                          Remove Hobby
                        </Button>
                      </div>
                    </div>
                  ...
                  <div className="w-full flex justify-end mt-2">
                    <Button onClick={handleAddHobby} className="ml-2">
                      Add Hobby
                    </Button>
                  </div>
                </div>
                ...
                <div>
                  {activityData.map((activity, index) => (
                    <div key={index} className="flex flex-wrap -mx-3 mb-6">
                      ...
                      <div className="w-full flex justify-end mt-2">
                        <Button color="red" onClick={() => handleRemoveActivity(index)} className="ml-2 py-2 px-2">
                          Remove Activity
                        </Button>
                      </div>
                    </div>
                  ...
                  <div className="w-full flex justify-end mt-2">
                    <Button onClick={handleAddActivity} className="ml-2">
                      Add Activity
                    </Button>
                  </div>
                </div>
