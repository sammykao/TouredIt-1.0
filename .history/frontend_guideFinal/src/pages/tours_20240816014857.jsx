...
export function Tours() {
  ...
  }, [email, confirmedTours, pendingTours]);

  const handleConfirm = async (id) => {
    try {
      await axios.post("http://localhost:3000/api/confirmTour", { id });
      setPendingTours(prev => {
        const confirmedTour = prev.find(tour => tour.id === id);
        setConfirmedTours(prevConfirmed => [...prevConfirmed, confirmedTour]);
                    ...
                    {confirmedTours.map((tour, index) => (
                      <li key={index} className="py-4 flex space-x-3">
                        <div className="flex-1 sm:px-6">
                          ...
                          <p className="text-md text-gray-500">{tour.comments}</p>
                    ...
                    {pendingTours.map((tour, index) => (
                      <li key={index} className="py-4 flex space-x-3">
                        <div className="flex-1 sm:px-6">
                          ...
                          <p className="text-md text-gray-800">+$40</p>
                          { tour.comments &&
                            <p className="text-md text-gray-700">Comments: {tour.comments}</p>
                          }
                          <div className="text-md text-gray-700">
                            <Typography className='font-bold text-md text-black my-1'>Add-on Package Requests</Typography>
                            {tour.greek_life && <p>Greek Life Experience (+$30)</p>}
                            {tour.student_athlete && <p>Student Athlete Life (+$30) </p>}
                            {tour.paid_internship && <p>Paid Internship Insights (+$30) </p>}
                          </div>
                        </div>
                      </li>
                    ))}
