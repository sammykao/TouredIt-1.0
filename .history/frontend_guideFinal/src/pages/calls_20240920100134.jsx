...
export function Calls() {
  ...
  const [confirmedCalls, setConfirmedCakks] = useState([]);
  ...
  useEffect(() => {
    ...
    axios.post("https://lera2ibwne.execute-api.us-east-2.amazonaws.com/api/retCalls", { email })
      .then(tourResponse => {
        setConfirmedCalls(tourResponse.data.confirmedCalls);
        setPendingCalls(tourResponse.data.nonConfirmedCalls);
        setLoading(false);
      })
      .catch(error => {
  ...
  const handleConfirm = async (id) => {
    try {
      axios.post("https://lera2ibwne.execute-api.us-east-2.amazonaws.com/api/confirmTour", { id });
      const confirmedTour = pendingCalls.find(tour => tour.id === id);
      setPendingCalls(prev => prev.filter(tour => tour.id !== id));
      setConfirmedCalls(prev => [...prev, confirmedTour]);
  ...
  const handleCancel = async (id) => {
    try {
      axios.post("https://lera2ibwne.execute-api.us-east-2.amazonaws.com/api/cancelTour", { id });
      setConfirmedCalls(prev => prev.filter(tour => tour.id !== id));
      window.location.href = "/calls";
    } catch (error) {
      setError(error);
    }
  ...
  const handleReject = async (id) => {
    try {
      axios.post("https://lera2ibwne.execute-api.us-east-2.amazonaws.com/api/declineTour", { id });
      setPendingCalls(prev => prev.filter(tour => tour.id !== id));
      window.location.href = "/calls";
    } catch (error) {
      setError(error);
          ...
          <div className="flex flex-col md:flex-row items-center">
            ...
            <div className="ml-0 md:ml-4 text-center md:text-left">
              ...
              <div className="mt-2 flex flex-col sm:flex-row sm:space-x-2">
                ...
                <Link to="/calls" className='bg-blue-500 hover:bg-blue-900 text-white font-bold py-1 px-3 rounded-md text-center'>
                  Calls
                </Link>
              </div>
            </div>
