...
export function UpdateHobbies() {
  ...
  const addHobby = async (e) => {
    e.preventDefault();
    try {
      const response = await fetch('https://lera2ibwne.execute-api.us-east-2.amazonaws.com/api/newHobby', {
        method: 'POST',
        headers: {
  ...
  const deleteHobby = async (id) => {
    if (window.confirm("Are you sure you want to delete this hobby?")) {
      try {
        ...
        const response = await axios.post('https://lera2ibwne.execute-api.us-east-2.amazonaws.com/api/remHobby', { 'hobby_id': id });
