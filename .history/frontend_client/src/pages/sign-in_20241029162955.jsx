...
export function SignIn() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isResetMode) {
        ...
        const { email, password } = formData;
        const local = await signIn(email, password);
        if (local && typeof local.AccessToken !== 'undefined') {
          localStorage.setItem('accessToken', local.AccessToken);
          if (!localStorage.getItem('accessToken')) {
            console.error('local token was not set properly.');
          }
        } else {
          console.error('SignIn local or AccessToken is undefined.');
        }
        setFormData({
          email: "",
          password: "",
        ...
        if (location.state && location.state.email && location.state.school) {
          navigate("/book-guide", { state: location.state });
        } else if (location.state && location.state.school && location.state.newformData && location.state.newSchools) {
          navigate(`/find-guide/${encodeURIComponent(location.state.school)}`, {state: location.state});
  ...
  return (
    <div className="relative isolate px-6 pt-16 lg:px-8 min-h-screen pb-24 bg-gray-500">
      <section className="mt-24 flex p-4 gap-4 bg-white rounded-3xl flex-wrap lg:flex-nowrap">
        <div className="w-full lg:w-3/5 mt-12 lg:mt-24">
          ...
          <form onSubmit={handleSubmit} className="mt-8 mb-2 mx-auto w-full max-w-screen-sm">
