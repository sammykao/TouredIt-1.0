...
const FindGuide = () => {
  ...
  const handleSubmit = async (e, newformData, newSchools) => {
    ...
    try {
      ...
      setFormData({
        ...
        // Stop loading here, after all operations
    }
  };
  ...
  useEffect(() => {
    if (location.state && location.state.newformData && location.state.newSchools) {
      ...
      location.state.newSchool = null;
    } else if (location.state && location.state.formData) {
      setLoading(true);
      handleCallSubmit(null, location.state.formData);
    }
  }, [location.state]);  // This will run once when the component mounts, and when `location.state` changes
