...
const TripMap = ({ schools }) => {
  ...
  const getCoordinates = async (places) => {
    ...
    if (filteredCoords.length > 0) {
      setMapCenter([filteredCoords[0].lat, filteredCoords[0].lon]); // Set map center to the first location's coordinates
    }
  };
  ...
  // Only render the map when mapCenter is available
  return (
    <div className="mt-40">
      <div
        className="mx-auto"
      ...
      >
        {mapCenter && (
          <MapContainer
            center={mapCenter} // Set the center dynamically
            zoom={7}
