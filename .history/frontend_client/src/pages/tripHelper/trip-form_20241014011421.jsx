...
const TripForm = ({
  ...
  // Submit handler with school and date concatenation
    ...
    // Concatenate school-date pairs into strings
        ...
        // Use a callback function inside `setFormData` to ensure formData is updated before proceeding with submission
        setTimeout(() => {
            console.log("Submitting FormData:", formData); // Check if `school` and `date` have been updated correctly
            try {
            handleSubmit(e);  // Call the original handleSubmit after state update
            } finally {
            setIsSubmitting(false);  // Stop loading when submit completes
            }
        }, 10); // Delay to ensure state updates before logging and submitting
    };
