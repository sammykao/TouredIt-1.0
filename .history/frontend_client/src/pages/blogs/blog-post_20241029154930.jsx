...
export function BlogPost() {
  const { title } = useParams(); // Get the post ID from the URL
  ...
  useEffect(() => {
    const fetchPost = async () => {
      try {
        ...
        const contentType = await environment.getContentType(import.meta.env.VITE_CONTENTFUL_CONTENT_ID); // Replace with actual content type ID

        // Log field IDs
        contentType.fields.forEach(field => {
          console.log(`Field name: ${field.name}, Field ID: ${field.id}`);
        });
        const response = await client.getEntries({
          content_type: import.meta.env.VITE_CONTENTFUL_CONTENT_ID,
          // 'fields.title': title,
          limit: 5,
        });
