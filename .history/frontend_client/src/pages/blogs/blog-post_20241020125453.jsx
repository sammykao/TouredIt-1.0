...
import { BLOCKS } from '@contentful/rich-text-types'; // Import block types for custom rendering
...
export function BlogPost() {
  ...
  const renderOptions = {
    renderNode: {
      ...
      [BLOCKS.PARAGRAPH]: (node, children) => <p className="text-lg sm:text-xl leading-relaxed mb-4">{children}</p>,
      [BLOCKS.UL_LIST]: (node, children) => <ul className="ml-6 mb-4">{children}</ul>,
      [BLOCKS.LIST_ITEM]: (node, children) => <li className="list-disc list-inside mb-2">{children}</li>,
    },
  };
