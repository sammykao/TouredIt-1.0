...
import { deleteUser, decrypt } from "./../cognitoConfig";
...
export function Profile() {
  ...
  const email = decrypt(sessionStorage.username;
  ...
  useEffect(() => {
    ...
    .then(response => {
      setConfirmedCalls(response.data.confirmedCalls);
      setPendingCalls(response.data.nonConfirmedCalls);
    })
    .catch(error => {
              ...
              <ul className="divide-y divide-gray-200">
                {confirmedCalls.map((call, index) => (
                  <li key={index} className="py-4 flex space-x-3">
                    <div className="flex-1 sm:px-6">
                      <div className="flex items-center justify-between">
                        <h3 className="text-md font-medium">{call.guide}</h3>
                        <p className="text-lg text-gray-700">{new Date(call.date).toLocaleDateString()}</p>
                      </div>
                      <p className="text-md text-gray-500">{call.school}</p>
                    </div>
                  </li>
                ))}
              </ul>
