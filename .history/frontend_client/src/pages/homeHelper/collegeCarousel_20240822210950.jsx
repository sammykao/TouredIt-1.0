        ...
        renderArrowPrev={(onClickHandler, hasPrev, label) =>
          hasPrev && (
            <button
              ...
              title={label}
              className="absolute top-1/2 transform -translate-y-1/2 left-4 bg-black bg-opacity-70 hover:bg-opacity-90 p-4 rounded-full z-10"
              style={{ zIndex: 10 }}
            >
              &lt;
            </button>
        ...
        renderArrowNext={(onClickHandler, hasNext, label) =>
          hasNext && (
            <button
              ...
              onClick={onClickHandler}
              title={label}
              className="absolute top-1/2 transform -translate-y-1/2 right-4 bg-black bg-opacity-70 hover:bg-opacity-90 p-4 rounded-full z-10"
              style={{ zIndex: 10 }}
            >
              &gt;
            </button>
        ...
        {schoolInfo.map((info, index) => (
          ...
          >
            <img
              src={info.logo_url}
              alt={`${info.name} logo`}
              className="h-40 object-contain rounded-lg mb-6"
            />
            ...
            <div className="flex items-center mb-2">
              ...
              <p className="text-lg text-gray-600">{getFirstWord(info.college_type)}</p>
            </div>
