  ...
  useEffect(() => {
    const fetchSchools = async () => {
      try {
        ...
        console.log(fetchedSchools.filter((item, index) => fetchedSchools.indexOf(item) !== fetchedSchools.lastIndexOf(item)));
      ...
      <div className="w-full mb-4">
        ...
        {filteredSchools.length > 0 && (
          <ul className="border bg-white border-gray-300 rounded mt-2 max-h-60 overflow-y-auto">
            {filteredSchools.map((schoo) => (
              <li
                key={`${school}`}  // Use a combination of school name and index for unique key
