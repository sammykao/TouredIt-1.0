...
export function SchoolCarousel() {
  ...
  useEffect(() => {
    ...
    const fetchSchoolInfo = async () => {
      try {
        const requests = topSchools.map(school =>
          axios.post(
        ...
        const fetchedData = responses.map(response => response.data.school);
      ...
      >
        {schoolInfo.map((info, index) => (
          ...
          >
            ...
            />
            <h2 className="text-5xl font-bold mb-6 text-gray-800">{info.name}</h2>
            ...
            <div className="grid grid-cols-2 gap-6 w-full">
              ...
              <div className="col-span-2 flex flex-col items-center p-6 bg-gray-100 rounded-lg shadow-inner">
                ...
                <p className="text-2xl text-gray-600">Location</p>
                <p className="text-3xl font-semibold text-gray-800">{info.location}</p>
              </div>
              ...
              <div className="flex flex-col items-center p-6 bg-gray-100 rounded-lg shadow-inner">
                ...
                <p className="text-2xl text-gray-600">Type</p>
                <p className="text-3xl font-semibold text-gray-800">{getFirstWord(info.type)}</p>
              </div>

              {/* Undergraduate Population Section */}
              <div className="flex flex-col items-center p-6 bg-gray-100 rounded-lg shadow-inner">
                <PeopleIcon className="text-purple-500 text-6xl mb-4" />
                <p className="text-2xl text-gray-600">Population</p>
                <p className="text-3xl font-semibold text-gray-800">{info.undergraduate_population.toLocaleString()}</p>
              </div>

              {/* In-State Tuition Section */}
              <div className="flex flex-col items-center p-6 bg-gray-100 rounded-lg shadow-inner">
                <AttachMoneyIcon className="text-yellow-500 text-6xl mb-4" />
                <p className="text-2xl text-gray-600">In-State Tuition</p>
                <p className="text-3xl font-semibold text-gray-800">{info.instate_tuition.toLocaleString()}</p>
              </div>

              {/* Out-of-State Tuition Section */}
              <div className="flex flex-col items-center p-6 bg-gray-100 rounded-lg shadow-inner">
                <AttachMoneyIcon className="text-red-500 text-6xl mb-4" />
                <p className="text-2xl text-gray-600">Out-of-State Tuition</p>
                <p className="text-3xl font-semibold text-gray-800">{info.outstate_tuition.toLocaleString()}</p>
              </div>
            </div>
          </div>
        ))}
