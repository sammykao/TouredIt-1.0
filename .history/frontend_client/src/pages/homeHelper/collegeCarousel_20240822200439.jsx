        ...
        renderArrowPrev={(onClickHandler, hasPrev, label) =>
          hasPrev && (
            <button
              ...
              title={label}
              className="absolute top-1/2 transform -translate-y-1/2 left-0 bg-black bg-opacity-70 hover:bg-opacity-90 p-2 rounded-full"
            >
              &lt;
            </button>
          )
        ...
        renderArrowNext={(onClickHandler, hasNext, label) =>
          hasNext && (
            <button
              ...
              onClick={onClickHandler}
              title={label}
              className="absolute top-1/2 transform -translate-y-1/2 right-0 bg-black bg-opacity-70 hover:bg-opacity-90 p-2 rounded-full"
            >
              &gt;
            </button>
          )
        ...
        {schoolInfo.map((info, index) => (
          ...
          >
            <img
              src={info.logo_url}
              alt={`${info.name} logo`}
              className="w-80 h-80 object-contain rounded-lg mb-6"
            />
