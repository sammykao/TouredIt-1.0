...
export function SchoolCarousel() {
  ...
  useEffect(() => {
    ...
    const isSchoolCacheValid = (schoolName) => {
      const cachedSchoolData = localStorage.getItem(`schoolData-${schoolName}`);
      if (!cachedSchoolData) return false;

      const { timestamp } = JSON.parse(cachedSchoolData);
      const cachedYear = new Date(timestamp).getFullYear();
      const currentYear = new Date().getFullYear();

      return cachedYear === currentYear;
    };

    // Fetch data sequentially for each school
    const fetchSchoolInfo = async () => {
      const fetchedData = [];

      for (let school of topSchools) {
        // Check cache first
        if (isSchoolCacheValid(school)) {
          const cachedData = JSON.parse(localStorage.getItem(`schoolData-${school}`));
          fetchedData.push(cachedData.data);
          continue;
        }

        // Fetch data if not in cache or expired
        try {
          const response = await axios.post("https://lera2ibwne.execute-api.us-east-2.amazonaws.com/api/schoolInfo", {
            name: school,
          });
  ...
  return (
    <div className="w-full max-w-5xl mx-auto my-10 relative">
      {schoolInfo.length > 0 && (
        <h1 className="text-4xl font-bold text-center mb-10 text-gray-800">Popular Colleges</h1>
      )}
      <Carousel
