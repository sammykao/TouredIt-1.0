...
const hasDuplicates = (array) => {
  ...
  return set.size !== array.length;  // If the set size is less, there are duplicates
};

// In your useEffect where you fetch schools, check for duplicates
useEffect(() => {
  const fetchSchools = async () => {
    try {
      ...
      // Check if there are duplicates in the fetched schools list
      if (hasDuplicates(fetchedSchools)) {
        console.warn('There are duplicates in the school list');
      }

      setSchools(fetchedSchools);  // Store the list of schools
      setLoading(false);
    } catch (error) {
