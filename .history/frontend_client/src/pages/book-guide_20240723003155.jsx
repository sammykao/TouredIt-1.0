...
const BookGuide = () => {
  ...
  useEffect(() => {
    const fetchGuideInfo = async () => {
      ...
      try {
        const response = await axios.post('https://naz5teqmllduknrtpdeor6ig5e0lfdhw.lambda-url.us-east-2.on.aws/api/guideInfo', { email });
        setGuideInfo(response.data.guide);
        const newImageUrl = `https://naz5teqmllduknrtpdeor6ig5e0lfdhw.lambda-url.us-east-2.on.aws/images/${response.data.guide.profile_image_url}`;
  ...
  const handleBookingSubmit = async (e) => {
    ...
    try {
      await axios.post('https://naz5teqmllduknrtpdeor6ig5e0lfdhw.lambda-url.us-east-2.on.aws/api/sendBookingRequest', 
      { data: bookingData, 
