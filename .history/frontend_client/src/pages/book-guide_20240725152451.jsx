...
const BookGuide = () => {
  ...
  const toggleExpandItem = (itemType, index) => {
    setExpandedItems(prev => ({
      ...prev,
      [itemType]: {
        ...prev[itemType],
        [index]: !prev[itemType]?.[index]
      }
    }));
  };
              ...
              <>
                ...
                <ul className="list-inside space-y-1">
                  {guideInfo.activities.map((activity, index) => (
                    <li key={index}>
                      <div className={expandedItems.activities?.[index] ? "" : "truncate overflow-hidden overflow-ellipsis"}>
                        {activity.name}: {activity.description}
                      </div>
                      <button
                        className="text-blue-500 hover:underline mt-2"
                        onClick={() => toggleExpandItem('activities', index)}
                      >
                        {expandedItems.activities?.[index] ? "Show Less" : "Show More"}
                      </button>
                    </li>
                  ))}
                </ul>
              ...
              <>
                ...
                <ul className="list-inside space-y-1">
                  {guideInfo.hobbies.map((hobby, index) => (
                    <li key={index}>
                      <div className={expandedItems.hobbies?.[index] ? "" : "truncate overflow-hidden overflow-ellipsis"}>
                        {hobby.name}: {hobby.description}
                      </div>
                      <button
                        className="text-blue-500 hover:underline mt-2"
                        onClick={() => toggleExpandItem('hobbies', index)}
                      >
                        {expandedItems.hobbies?.[index] ? "Show Less" : "Show More"}
                      </button>
                    </li>
                  ))}
                </ul>
