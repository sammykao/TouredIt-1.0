      ...
      >
        ...
        <div className="p-4">
          {(guideInfo?.greek_life || guideInfo?.student_athlete || guideInfo?.paid_internship) && (
            <>
              <Typography className='text-md font-bold text-gray-700 mb-4'>Add packages to your request</Typography>
              <div className="mb-4 space-y-4">
                {guideInfo?.greek_life && (
                  <div
                    className={`border p-4 rounded-lg cursor-pointer ${
                      bookingData.greek_life ? 'bg-yellow-500 border-yellow-600' : 'bg-white border-gray-300'
                    }`}
                    onClick={() => togglePackage('greek_life')}
                  >
                    <div className="flex items-center">
                      <SportsBarIcon className={`${bookingData.greek_life ? 'text-white' : 'text-gray-700'}`} />
                      <div className="ml-4">
                        <h3 className={`font-bold ${bookingData.greek_life ? 'text-white' : 'text-gray-700'}`}>Greek Life Experience</h3>
                        <p className={`text-sm ${bookingData.greek_life ? 'text-gray-200' : 'text-gray-500'}`}>Learn about the greek life on campus, the rush process, and potentially even see the guide's respective frat or sorority house(s).</p>
                        <p className={`text-sm font-bold ${bookingData.greek_life ? 'text-white' : 'text-gray-700'}`}>+$15</p>
                      </div>
                    </div>
                  </div>
                )}
                {guideInfo?.student_athlete && (
                  <div
                    className={`border p-4 rounded-lg cursor-pointer ${
                      bookingData.student_athlete ? 'bg-yellow-500 border-yellow-600' : 'bg-white border-gray-300'
                    }`}
                    onClick={() => togglePackage('student_athlete')}
                  >
                    <div className="flex items-center">
                      <EmojiEventsIcon className={`${bookingData.student_athlete ? 'text-white' : 'text-gray-700'}`} />
                      <div className="ml-4">
                        <h3 className={`font-bold ${bookingData.student_athlete ? 'text-white' : 'text-gray-700'}`}>Student Athlete Add-On</h3>
                        <p className={`text-sm ${bookingData.student_athlete ? 'text-gray-200' : 'text-gray-500'}`}>Learn about the recruiting process, balancing academics and athletics, and potentially even see athlete exclusive facilities.</p>
                        <p className={`text-sm font-bold ${bookingData.student_athlete ? 'text-white' : 'text-gray-700'}`}>+$20</p>
                      </div>
                    </div>
                  </div>
                )}
                {guideInfo?.paid_internship && (
                  <div
                    className={`border p-4 rounded-lg cursor-pointer ${
                      bookingData.paid_internship ? 'bg-yellow-500 border-yellow-600' : 'bg-white border-gray-300'
                    }`}
                    onClick={() => togglePackage('paid_internship')}
                  >
                    <div className="flex items-center">
                      <WorkIcon className={`${bookingData.paid_internship ? 'text-white' : 'text-gray-700'}`} />
                      <div className="ml-4">
                        <h3 className={`font-bold ${bookingData.paid_internship ? 'text-white' : 'text-gray-700'}`}>Internship Insights</h3>
                        <p className={`text-sm ${bookingData.paid_internship ? 'text-gray-200' : 'text-gray-500'}`}>Your guide has noted paid internship experience. Learn about the internship recruitment process, how to get internships as a college student, 
                          and ask any tips for resume or on-campus activities.</p>
                        <p className={`text-sm font-bold ${bookingData.paid_internship ? 'text-white' : 'text-gray-700'}`}>+$25</p>
                      </div>
                    </div>
                  </div>
                )}
