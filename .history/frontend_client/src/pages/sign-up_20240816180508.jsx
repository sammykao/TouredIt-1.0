...
export function SignUp() {
  ...
  const insertBackend = async () => {
    ...
    try {
      const response = await axios.post('http://localhost:3000/api/newClient', newAccount);
      return;
    } catch (error) {
      console.error('Error Inserting Account:', error);
    }
    try {
      const response = await axios.post('http://localhost:3000/api/updateClient', newAccount);
      return;
    } catch (error) {
