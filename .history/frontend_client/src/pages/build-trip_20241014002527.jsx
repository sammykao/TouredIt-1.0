...
export function BuildTrip() {
  ...
  const [schoolDropdown, setSchoolDropdown] = useState(null);
  const [schools, setSchools] = useState(null);

  useEffect(() => {
    console.log("Inside useEf fect");  // This should print
  
    const fetchSchools = async () => {
      console.log("Inside fetchSchools function");  // This should print if fetchSchools is called
      try {
        console.log("Starting to fetch schools...");  // This should print if try block starts
        ...
        const duplicates = fetchedSchools.filter(
          (item, index) => fetchedSchools.indexOf(item) !== fetchedSchools.lastIndexOf(item)
        );
        ...
        fetchedSchools = [...new Set(fetchedSchools)]; // Remove duplicates
        setSchoolDropdown(fetchedSchools);
      } catch (error) {
        console.error('Error fetching schools:', error.message);
      }
    };
  
    fetchSchools();  // This should trigger the function
  }, []);
  
  
  console.log("Outside useEffect");  // This should fire immediately when the component renders
          ...
          tripForm && (
            <div className="bg-white mt-40 px-6 py-6 rounded-lg mx-auto">
              <div className="w-full mb-6 text-center">
                <h1 className="text-2xl sm:text-4xl leading-7 text-black">Hi! Let's build a trip. </h1>
                <TripForm
                      formData={formData}
