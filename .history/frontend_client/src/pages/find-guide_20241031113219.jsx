...
import { decrypt } from "@/cognitoConfig";
import ReactGa from "reac"
...
const FindGuide = () => {
  ...
  const handleSubmit = async (e, newformData, newSchools) => {
    ...
    try {
      await axios.post('https://lera2ibwne.execute-api.us-east-2.amazonaws.com/api/sendCustomRequest', { data: newformData, email: decrypt(localStorage.username) });
      setTripForm(false);
      setShowMap(true);
    } catch (error) {
      ...
      const userInfoResponse = await axios.post('https://lera2ibwne.execute-api.us-east-2.amazonaws.com/api/accountInfo', { email: decrypt(localStorage.username) });
  ...
  const handleCallSubmit = async (e, callformData) => {
    ...
    try {
        await axios.post('https://lera2ibwne.execute-api.us-east-2.amazonaws.com/api/sendCustomCallRequest', { data: callformData, email: decrypt(localStorage.username) });
        setFormData({
            school: school,
            major: '',
            grade: '',
        ...
        <>
          ...
          {showForm ? (
            ...
            <>
              ...
              <div className="grid mt-12 gap-6 grid-cols-1 md:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 pt-6">
