...
export function BuildTrip() {
  ...
  const handleSubmit = async (e, newformData, newSchools) => {
    ...
    try {
      ...
      const { data: userInfo } = axios.post(
        'https://lera2ibwne.execute-api.us-east-2.amazonaws.com/api/accountInfo',
      ...
      // Call personalize API with form data string
      const { data: personalizedResponse } = await axios.post(
        'https://en7d12lkv8.execute-api.us-east-2.amazonaws.com',
        { prompt: completeString }
      );
      console.log( personalizedResponse );
      // Set the personalized message to be displayed
      setPersonalizedMessage(personalizedResponse.message.content[0].text);

      // Reset form data and update the view
      setFormData({
        school: '',
        major: '',
        grade: '',
          ...
          showMap && (
            <div className="bg-white mt-40 px-6 py-6 rounded-lg mx-auto">
              <div className="w-full mb-6 text-center">
                  ...
                  <div className="mt-10">
                    <h2 className="text-xl text-black font-bold">Personalized Message:</h2>
                    <p className="text-md text-black mt-4">{personalizedMessage}</p>
                  </div>
              </div>
            </div>
          )
