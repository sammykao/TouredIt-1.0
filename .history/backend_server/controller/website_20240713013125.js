...
exports.sendEmail = async (req, res) => {
    ...
    const mailOptions = {
        ...
        to: 'kaosam77@gmail.com', // receiver address
...
exports.getClientTours = async (req, res) => {
    try {
        const { id } = req.body; // Assuming req.body contains the client ID
        ...
        const query = `
            SELECT 
                t.id, t.event_date, t.event_time, t.school, t.completed, 
                t.feedback, t.confirmed, tg.name AS tour_guide_name
            FROM 
                tours t
            INNER JOIN 
        ...
        if (result.rows.length === 0) {
            ...
            res.status(200).json({ tours: result.rows });
        }
