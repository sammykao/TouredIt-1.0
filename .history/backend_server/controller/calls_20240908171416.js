...
exports.confirmGuideCall = async (req, res) => {
    try {
        ...
        const callQuery = "SELECT tour_guide_id, client_id, event_date, greek_life, student_athlete, paid_internship FROM calls WHERE id = $1";
        const callResult = await db.query(callQuery, [id]);
        if (callResult.rows.length === 0) {
            res.status(404).json({ message: 'Call not found' });
            return;
        ...
        const googleMeetLink = await createGoogleMeetSpace(authClient);
        ...
        let mailOptions = {
            ...
            &#8226; Google Meet Link: <a href="${googleMeetLink}">Join Meeting</a><br><br> 
        ...
        mailOptions = {
            ...
            &#8226; Google Meet Link: <a href="${googleMeetEvent.hangoutLink}">Join Meeting</a><br><br> 
            <br><br>
            ...
            &#8226; <strong> Date and School:</strong>"I'm excited to talk to you or your child about ${guideResult.rows[0].school} on ${tourResult.rows[0].event_date}."<br>\t\t\t
...
async function createGoogleMeetEvent(guideEmail) {
    try {
        const calendar = google.calendar({ version: 'v3', auth: oauth2Client });

        // Get the current time for an immediate event
        const startTime = new Date().toISOString();
        const endTime = new Date(new Date().getTime() + 60 * 60 * 1000).toISOString(); // 1 hour from now

        const event = {
            summary: 'Video Chat with Client',
            description: 'A video chat scheduled between the guide and client.',
            start: {
                dateTime: startTime, // Use the current time
                timeZone: 'America/New_York', // Adjust based on your timezone
            },
            end: {
                dateTime: endTime, // 1 hour from now
                timeZone: 'America/New_York',
            },
            attendees: [{ email: guideEmail }],
            conferenceData: {
                createRequest: {
                    requestId: "sample123",
...
const SERVICE_ACCOUNT_KEY_FILE = 'path/to/service-account-key.json';
...
async function createGoogleMeetSpace(authClient) {
    ...
    return response[0].meetingUri; // Returns the Google Meet URL
}