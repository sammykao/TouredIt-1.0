...
// Helper function to generate JWT Token for Zoom Video SDK
const generateZoomVideoSDKToken = () => {
    const payload = {
        app_key: SDK_KEY,
        version: 1,
        tpc: "TouredIt_Video_Session", // Unique session name/topic for the call
    };
    
    // Sign the token with SDK Secret
    const token = jwt.sign(payload, SDK_SECRET, { algorithm: 'HS256', expiresIn: '1h' });
    
    return token;
};

// Create a new Zoom Video SDK session and return the token and session details
const createZoomVideoSession = async (guide_email) => {
    try {
        const token = generateZoomVideoSDKToken();
        
        // Optionally, you can send this token to the frontend or store the session info in your database.
        return {
            token,
            sessionName: "TouredIt_Video_Session",
            guide_email,
        };

    } catch (error) {
        console.error("Error creating Zoom Video SDK session:", error);
        throw new Error("Could not create Zoom Video SDK session.");
    }
};
...
exports.confirmGuideCall = async (req, res) => {
    try {
        ...
        const videoSession = await createZoomVideoSession(guideResult.rows[0].email);

        // Send session token and info to the client via email
        let mailOptions = {
            from: process.env.EMAIL_USERNAME,
            to: clientResult.rows[0].email,
            subject: "Your video chat session is ready",
            html: `<p>Hey ${clientResult.rows[0].name},<br><br>
            Good news, ${guideResult.rows[0].name} has accepted your video chat request.
            Please join the video call at the scheduled time using the session details below.<br><br>
            <strong>Session Name:</strong> ${videoSession.sessionName}<br>
            <strong>Guide:</strong> ${guideResult.rows[0].name}<br><br>
            <strong>Join the session using this link:</strong> <a href="https://yourdomain.com/video/${videoSession.sessionName}">Join Video Call</a><br><br>
            If you have any questions, feel free to email info@touredit.com.<br><br>
            Warm regards,<br>
            <strong>TouredIt Team</strong>
            </p>`
        };
        ...
        mailOptions = {
            from: process.env.EMAIL_USERNAME,
            to: guideResult.rows[0].email,
            subject: "You have a new video chat session",
            html: `<p>Hey ${guideResult.rows[0].name},<br><br>
            You have been assigned a new video chat session with ${clientResult.rows[0].name}. Please join the session at the scheduled time.<br><br>
            <strong>Session Name:</strong> ${videoSession.sessionName}<br><br>
            <strong>Join the session using this link:</strong> <a href="https://yourdomain.com/video/${videoSession.sessionName}">Join Video Call</a><br><br>
            If you have any questions, feel free to email info@touredit.com.<br><br>
            Warm regards,<br>
            <strong>TouredIt Team</strong>
        ...
        // Optionally, you can store the session details in your database if necessary
        res.status(200).json({ message: 'Call request confirmed successfully', call: insertResult.rows[0], session: videoSession });
