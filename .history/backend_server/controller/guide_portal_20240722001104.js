...
exports.confirmTour = async (req, res) => {
    try {
        ...
        if (!email || !data.tour_it || !guide_email || !school) {
        ...
        const updateQuery = `UPDATE tours SET confirmed = true WHERE id = $1`;

        const insertResult = await db.query(updateQuery, [data.]);
