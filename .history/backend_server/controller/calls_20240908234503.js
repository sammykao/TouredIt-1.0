...
exports.confirmGuideCall = async (req, res) => {
    try {
        ...
        const googleMeetLink = await createGoogleMeetLink([clientResult.rows[0].email, guideResult.rows[0].email], callResult.rows[0].event_date); // Example client email
...
const createGoogleMeetLink = async (attendees, date) => {
    try {
        ...
        const startTime = `${date[2] + "-" + date[0] + "-" + date[1]}T23:59:00`;
        const endTime = `${date[2] + "-" + date[0] + "-" + date[1]}T00:00:00`;

        const event = {
            summary: "TouredIt Video Chat",
            description: "Video chat with tour guide",
            start: {
                dateTime: startTime,
