...
exports.confirmTour = async (req, res) => {
    try {
        const { data, email, guide_email } = req.body;
        ...
        const guideName = guideResult.rows[0].name;
        const guideSchool = guideResult.rows[0].school;
        ...
        let mailOptions = {
            ...
            ${guideName} has accepted your tour request for ${guideSchool} on ${data.date}. They will reach out to arrange and coordinate on logistics and timing.<br><br>
            Warmly,<br>
            <strong>TouredIt Team</strong></p>`
        };
        ...
        mailOptions = {
            ...
            html: `<p>Hi ${guideName},<br><br>
            Congrats on being selected to give a tour! Here are the client details:<br>
            - Client name: ${clientName}<br>
            - Client email: ${email}<br>
            - Client phone: ${clientResult.rows[0].phone}<br>
            - Tour Date: ${data.date}<br>
            - Tour comments: ${insertResult.rows[0].comments}<br><br>
            Please contact the client to arrange the details.<br><br>
            Warmly,<br>
            <strong>TouredIt Team</strong></p>`
        };
        ...
        transporter.sendMail(mailOptions, function (err, info) {
            if (err) {
                console.error('Error sending email to guide:', err);
            }
        });
        ...
        mailOptions = {
            ...
            ${guideName} has accepted a tour request from ${clientName} on ${data.date} for ${guideSchool}.<br><br>
            Warmly,<br>
            <strong>TouredIt Team</strong></p>`
        };
...
exports.deleteTour = async (req, res) => {
    try {
        ...
        const updateQuery = `DELETE FROM tours WHeRE id = $1`;
        const insertResult = await db.query(updateQuery, [data.tour_id]);
