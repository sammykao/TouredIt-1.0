...
exports.confirmGuideCall = async (req, res) => {
    try {
        ...
        const zoomLink = await createZoomMeeting(guideResult.rows[0].email);
        ...
        let mailOptions = {
            ...
            &#8226; Zoom Link: <a href="${zoomLink}">${zoomLink}</a><br><br> 
        ...
        mailOptions = {
            ...
            &#8226; Zoom Meet Link: <a href="${zoomLink}">${zoomLink}</a><br><br> 
            \t &#8226; <strong> The guide details </strong><br>\t\t
            &#8226; <strong> Guide name:</strong> ${guideResult.rows[0].name}<br>\t\t
            &#8226; <strong> Guide email:</strong> ${guideResult.rows[0].email}<br>\t\t
            &#8226; <strong> Guide School: </strong> ${guideResult.rows[0].school} <br><br>
...
// Fetch the access token using your Zoom Server-to-Server OAuth credentials
const getZoomAccessToken = async () => {
    try {
        const authData = `${process.env.ZOOM_CLIENT_ID}:${process.env.ZOOM_CLIENT_SECRET}`;
        ...
        throw new Error("Could not fetch Zoom access token.");
    }
};

const createZoomMeeting = async (guide_email) => {
    try {
        const token = await getZoomAccessToken();


        console.log(token);
        // Create a Zoom meeting that allows participants to join without the host
        const response = await axios.post(
            'https://api.zoom.us/v2/users/me/meetings',
            {
                topic: "TouredIt Video Chat",
                type: 1,  // Instant meeting
                settings: {
                    join_before_host: true,
                    host_video: true,
                    participant_video: true, 
                    alternative_hosts: `${guide_email}`
                }
            },
            {
                headers: {
