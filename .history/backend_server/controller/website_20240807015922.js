...
exports.insertClientAccounts = async (req, res) => {
    try {
        ...
        if (!isValidAccount(account)) {
            res.status(400).send(JSON.stringify({ message: 'Invalid account data' }));
            return;
        }
        ...
        res.status(201).send(JSON.stringify({ message: 'Account created', account: result.rows[0] }));
...
exports.retrieveClientInfo = async (req, res) => {
    try {
        ...
        if (result.rows.length === 0) {
            ...
            res.status(200).send(JSON.stringify({ account: result.rows[0] }));
        }
...
exports.updateClientInfo = async (req, res) => {
    try {
        ...
        const query = "UPDATE clients SET name = COALESCE($2, name), phone = COALESCE($3, phone) WHERE email = $1 RETURNING *";
        const values = [account.email, account.name, account.phone];
        const result = await db.query(query, values);
        ...
        if (result.rows.length === 0) {
            ...
            res.status(200).send(JSON.stringify({ message: 'Account updated', account: result.rows[0] }));
        }
...
exports.getAllGuides = async (req, res) => {
    try {
        ...
        if (!search.school) {
            res.status(400).send(JSON.stringify({ message: 'Must provide a school to get guides for' }));
            return;
        }
        ...
        if (result.rows.length === 0) {
            res.status(404).send(JSON.stringify({ message: "No guides found for the specified college" }));
        } else {
            res.status(200).send(JSON.stringify({ guides: result.rows }));
        }
...
exports.getGuideInfo = async (req, res) => {
    try {
        ...
        if (!guide.email) {
            res.status(400).send(JSON.stringify({ message: 'Must provide an email to get guide' }));
            return;
        }
        ...
        const query = `
            SELECT 
                ...
                COALESCE(json_agg(jsonb_build_object('name', ca.activity_name, 'description', ca.description)) FILTER (WHERE ca.activity_name IS NOT NULL), '[]') AS activities,
                COALESCE(json_agg(jsonb_build_object('name', h.hobby_name, 'description', h.description)) FILTER (WHERE h.hobby_name IS NOT NULL), '[]') AS hobbies
            FROM 
                tour_guides tg
            LEFT JOIN 
        ...
        if (result.rows.length === 0) {
            res.status(404).send(JSON.stringify({ message: "Guide Not Found" }));
        } else {
            const guideInfo = {
            ...
            res.status(200).send(JSON.stringify({ guide: guideInfo }));
        }
    } catch (error) {
        res.status(500).send(JSON.stringify({ message: error.message }));
    }
...
exports.getGuidesByFilter = async (req, res) => {
    try {
        ...
        if (result.rows.length === 0) {
            res.status(404).send(JSON.stringify({ message: "No guides found with the specified filters" }));
        } else {
            res.status(200).send(JSON.stringify({ guides: result.rows }));
        }
    } catch (error) {
...
exports.getAllSchoolNames = async (req, res) => {
    try {
        ...
        if (result.rows.length === 0) {
            res.status(404).send(JSON.stringify({ message: "No schools found" }));
        } else {
            const schoolNames = result.rows.map(school => school.name);
            res.status(200).send(JSON.stringify({ schools: schoolNames }));
        }
...
exports.getSchoolInfo = async (req, res) => {
    try {
        ...
        if (result.rows.length === 0) {
            res.status(404).send(JSON.stringify({ message: "School Not Found" }));
        } else {
            res.status(200).send(JSON.stringify({ school: result.rows[0] }));
        }
...
exports.getAllSchoolInfos = async (req, res) => {
    try {
        ...
        if (result.rows.length === 0) {
            ...
            res.status(200).send(JSON.stringify({ schools: result.rows }));
        }
...
exports.sendEmail = async (req, res) => {
    ...
    if (emailResult.success) {
        ...
        res.status(500).send(JSON.stringify({ message: 'Error sending email', error: emailResult.error }));
    }
};
...
exports.getClientTours = async (req, res) => {
    try {
        ...
        if (!email) {
            res.status(400).send(JSON.stringify({ message: 'Invalid client email' }));
            return;
        }
        ...
        const result = await db.query(query, values);
        if (result.rows.length === 0) {
            res.status(404).send(JSON.stringify({ message: "No tours found for the specified client" }));
        } else {
            ...
            res.status(200).send(JSON.stringify({ confirmedTours, nonConfirmedTours }));
        }
...
exports.sendcustomtour = async (req, res) => {
    try {
        ...
        if (!email || !data) {
            res.status(400).send(JSON.stringify({ message: 'Invalid client request' }));
            return;
        }
        ...
        if (!emailResult.success) {
            res.status(500).send(JSON.stringify({ message: 'Error sending email to client', error: emailResult.error }));
            return;
        }
        ...
        if (!emailResult.success) {
            res.status(500).send(JSON.stringify({ message: 'Error sending email to TouredIt team', error: emailResult.error }));
            return;
        }

        res.status(200).send(JSON.stringify({ message: 'Custom request submitted successfully' }));
...
exports.sendtour = async (req, res) => {
    try {
        ...
        if (guideResult.rows.length === 0 || clientResult.rows.length === 0) {
            res.status(404).send(JSON.stringify({ message: "Guide or client not found" }));
            return;
        }
        ...
        emailResult = await sendEmailWithRetry(mailOptions);
        if (!emailResult.success) {
            res.status(500).send(JSON.stringify({ message: 'Error sending email to guide', error: emailResult.error }));
            return;
        }

        res.status(200).send(JSON.stringify({ message: 'Tour request submitted successfully', tour: insertResult.rows[0] }));
