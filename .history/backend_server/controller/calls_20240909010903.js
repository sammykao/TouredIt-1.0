...
exports.confirmGuideCall = async (req, res) => {
    try {
        ...
        let mailOptions = {
            ...
            &#8226; Zoom Link: <a href="${zoomLink}"></a><br><br> 
        ...
        mailOptions = {
            ...
            &#8226; Zoom Meet Link: <a href="${zoomLink}"></a><br><br> 
            \t &#8226; <strong> The guide details </strong><br>\t\t
            &#8226; <strong> Guide name:</strong> ${guideResult.rows[0].name}<br>\t\t
            &#8226; <strong> Guide email:</strong> ${guideResult.rows[0].email}<br>\t\t
            &#8226; <strong> Guide School: </strong> ${guideResult.rows[0].school} <br><br>
...
const getZoomAccessToken = async () => {
    try {
        const authData = `${process.env.ZOOM_CLIENT_ID}:${password: process.env.ZOOM_CLIENT_SECRET}`;
        const response = await axios.post(`https://zoom.us/oauth/token?grant_type=account_credentials&account_id=${process.env.ZOOM_ACCOUNT_ID}`,
            null, { 
            auth: {
                username: process.env.ZOOM_CLIENT_ID, // Your Zoom Client ID
                password: process.env.ZOOM_CLIENT_SECRET, // Your Zoom Client Secret
            },
        });
        ...
        const response = await axios.post(
            'https://api.zoom.us/v2/users/me/meetings',
            {
                ...
                settings: {
                    ...
                    join_before_host: true  // Allow participants to join without the host
                }
            },
            {
                headers: {
