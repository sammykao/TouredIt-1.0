...
exports.confirmTour = async (req, res) => {
    try {
        ...
        const tourResult = await db.query(guideQuery, [guide_email]);


        const guideQuery = "SELECT email, name, school FROM tour_guides WHERE email = $1";
        const clientQuery = "SELECT email, name, phone FROM clients WHERE email = $1";
