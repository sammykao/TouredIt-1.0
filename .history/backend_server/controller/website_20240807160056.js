...
exports.sendtour = async (req, res) => {
    try {
        ...
        let mess = "Guide or client not found. Guide: " + (guideResult.rows[0] ? JSON.stringify(guideResult.rows[0]) : "none") + ", Client: " + (clientResult.rows[0] ? JSON.stringify(clientResult.rows[0]) : "none");

        if (guideResult.rows.length === 0 || clientResult.rows.length === 0) {
            res.status(404).json({ message: mess });
            return;
        }
