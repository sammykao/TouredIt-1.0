...
exports.confirmGuideCall = async (req, res) => {
    try {
        ...
        console.log(callResult.rows[0].event_date);
        const googleMeetLink = await createGoogleMeetLink(callResult.rows[0].event_date); // Example client email
...
const createGoogleMeetLink = async (date) => {
    try {
        ...
        const calendar = new calendar_v3.Calendar({ version: "v3", auth });

        date = date.split("/");
        // Format the start and end times for 11:59 PM to 12:00 AM (1 minute)
        const startTime = `${date[2] + "-" + date[0] + "-" + date[1]}T12:00:00`;
        const endTime = `${date[2] + "-" + date[0] + "-" + date[1]}T12:01:00`;

        const event = {
            summary: 'TouredIt Video Chat',
            location: 'Online',
            description: 'Video chat with student',
            start: {
                dateTime: startTime,
                timeZone: 'America/New_York', // Adjust the time zone if needed
            },
            end: {
                dateTime: endTime,
                timeZone: 'America/New_York', // Adjust the time zone if needed
            ...
            conferenceData: {
                createRequest: {
                    requestId: "unique-request-id", // Ensure this is unique per request
        ...
        const response = await axios.post(
            'https://api.zoom.us/v2/users/me/meetings',
            {
                ...
                settings: {
                    ...
                    join_before_host: true  // This allows participants to join without the host
                }
            },
            {
                headers: {
