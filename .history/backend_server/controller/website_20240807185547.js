    ...
    try {
        ...
        const query = `
            SELECT 
                ...
                COALESCE(json_agg(jsonb_build_object('name', ca.activity_name, 'description', ca.description, 'id', ca.id)) FILTER (WHERE ca.activity_name IS NOT NULL), '[]') AS activities,
                COALESCE(json_agg(jsonb_build_object('name', h.hobby_name, 'description', h.description, 'id', h.hobby_id)) FILTER (WHERE h.hobby_name IS NOT NULL), '[]') AS hobbies
            FROM 
                tour_guides tg
            LEFT JOIN 
