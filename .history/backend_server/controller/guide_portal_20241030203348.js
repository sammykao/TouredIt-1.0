...
exports.addHobbies = async (req, res) => {
    try {
        ...
        const query = `
            ...
            SELECT id, $2, $3
            FROM guide
            ON CONFLICT (tourguide_id, hobby_name)
        ...
        res.status(201).json({ message: 'Hobby added or updated', hobby: result.rows[0] });
...
exports.addInvolvement = async (req, res) => {
    try {
        ...
        const query = `
            ...
            SELECT id, $2, $3
            FROM guide
            ON CONFLICT (tourguide_id, activity_name)
            DO UPDATE SET description = EXCLUDED.description
            RETURNING *;
        `;
        ...
        const result = await db.query(query, values);
        res.status(201).json({ message: 'Involvement added or updated', involvement: result.rows[0] });
...
exports.addWorkExp = async (req, res) => {
    try {
        ...
        const query = `
            ...
            SELECT id, $2, $3
            FROM guide
            ON CONFLICT (tourguide_id, job_name)
            DO UPDATE SET description = EXCLUDED.description
            RETURNING *;
        `;
        ...
        const result = await db.query(query, values);
        res.status(201).json({ message: 'Job added or updated', work_experience: result.rows[0] });
