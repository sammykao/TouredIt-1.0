...
exports.insertOrUpdateGuideAccount = async (req, res) => {
    try {
        ...
        const checkQuery = `SELECT * FROM tour_guides WHERE LOWER(email) = LOWER($1)`;
        const checkResult = await db.query(checkQuery, [account.email]);

        if (checkResult.rows.length > 0) {
            // Account exists, call updateGuideAccounts
            return exports.updateGuideAccounts(req, res);
        } else {
            // Account does not exist, insert new account
            const insertQuery = `
                INSERT INTO 
                ...
                RETURNING *`;
            const values = [
                account.name, account.email, account.school, account.hometown,
            ...
            res.status(201).json({ message: 'Account created', account: insertResult.rows[0] });
        }

    } catch (error) {
        console.error('Error inserting or updating guide account:', error);
        res.status(500).json({ message: 'Internal server error' });
    }
};

orts.addHobbies = async (req, res) => {
    try {
        ...
        const guideResult = await db.query(guideQuery, [email]);
        ...
        const tourguide_id = guideResult.rows[0].id;

        const checkQuery = `SELECT 1 FROM hobbies WHERE tourguide_id = $1 AND hobby_name = $2`;
        const checkResult = await db.query(checkQuery, [tourguide_id, hobby_name]);

        if (checkResult.rows.length > 0) {
            const updateQuery = `UPDATE hobbies SET description = $3 WHERE tourguide_id = $1 AND hobby_name = $2`;
            await db.query(updateQuery, [tourguide_id, hobby_name, description]);
        } else {
            const insertQuery = `INSERT INTO hobbies (tourguide_id, hobby_name, description) VALUES ($1, $2, $3)`;
            await db.query(insertQuery, [tourguide_id, hobby_name, description]);
        }

        res.status(201).json({ message: 'Hobby added or updated' });
...
exports.addInvolvement = async (req, res) => {
    try {
        ...
        const checkQuery = `SELECT 1 FROM campus_involvement WHERE tourguide_id = $1 AND activity_name = $2`;
        const checkResult = await db.query(checkQuery, [tourguide_id, activity_name]);

        if (checkResult.rows.length > 0) {
            const updateQuery = `UPDATE campus_involvement SET description = $3 WHERE tourguide_id = $1 AND activity_name = $2`;
            await db.query(updateQuery, [tourguide_id, activity_name, description]);
        } else {
            const insertQuery = `INSERT INTO campus_involvement (tourguide_id, activity_name, description) VALUES ($1, $2, $3)`;
            await db.query(insertQuery, [tourguide_id, activity_name, description]);
        }

        res.status(201).json({ message: 'Involvement added or updated' });
...
exports.addWorkExp = async (req, res) => {
    try {
        ...
        const tourguide_id = guideResult.rows[0].id;
        const checkQuery = `SELECT 1 FROM work_experience WHERE tourguide_id = $1 AND job_name = $2`;
        const checkResult = await db.query(checkQuery, [tourguide_id, job_name]);

        if (checkResult.rows.length > 0) {
            const updateQuery = `UPDATE work_experience SET description = $3 WHERE tourguide_id = $1 AND job_name = $2`;
            await db.query(updateQuery, [tourguide_id, job_name, description]);
        } else {
            const insertQuery = `INSERT INTO work_experience (tourguide_id, job_name, description) VALUES ($1, $2, $3)`;
            await db.query(insertQuery, [tourguide_id, job_name, description]);
        }

        res.status(201).json({ message: 'Job added or updated' });
...
exports.deleteWorkExp = async (req, res) => {
    try {
        ...
        const query = 'DELETE FROM work_experience WHERE id = $1 RETURNING *';
        const values = [id];
...
exports.retrieveTours = async (req, res) => {
    try {
        ...
        const query = `
            SELECT
                t.event_date AS date, t.school, c.name,
                c.email, c.phone, t.confirmed, t.id AS id, 
                t.greek_life, t.student_athlete, t.paid_internship, t.comments
            FROM
                tours t
            INNER JOIN
    ...
    try {
        ...
        const query = `
            ...
            SET
                ...
                student_athlete = COALESCE($16, student_athlete),
                paid_internship = COALESCE($17, paid_internship)
            WHERE 
                LOWER(email) = LOWER($2)
            RETURNING *`;
        const values = [account.name, account.email, account.school, account.hometown, 
                        account.phone, account.bio, account.major, account.secondary_major, 
                        account.minor, account.secondary_minor, account.profile_image_url, 
                        account.instagram, account.linkedin, account.grad_year,
                        account.greek_life, account.student_athlete, account.paid_internship];
...
exports.retrieveGuideInfo = async (req, res) => {
    try {
        ...
        if (!email) {
            return res.status(400).json({ message: 'Must provide an email to get guide' });
        }
        ...
        const activitiesResult = await db.query(activityQuery, [guide.id]);
        const hobbiesResult = await db.query(hobbyQuery, [guide.id]);
        const workExpResult = await db.query(workExpQuery, [guide.id]);
        ...
        const response = {
            ...
            bio: guide.bio,
            major: guide.major,
            secondary_major: guide.secondary_major,
            minor: guide.minor,
            secondary_minor: guide.secondary_minor,
            profile_image_url: guide.profile_image_url,
            instagram: guide.instagram,
            linkedin: guide.linkedin,
            num_tours: guide.num_tours,
            greek_life: guide.greek_life,
            student_athlete: guide.student_athlete,
            paid_internship: guide.paid_internship,
            grad_year: guide.grad_year,
            activities: activitiesResult.rows,
            hobbies: hobbiesResult.rows,
            work_experience: workExpResult.rows,
        };

        return res.status(200).json({ guide: response });
...
exports.confirmTour = async (req, res) => {
    try {
        ...
        const tourQuery = "SELECT tour_guide_id, client_id, event_date, greek_life, student_athlete, paid_internship FROM tours WHERE id = $1";
        ...
        let additionalText = "";
        if (tourResult.rows[0].greek_life) {
        ...
        if (tourResult.rows[0].student_athlete) {
        ...
        if (tourResult.rows[0].paid_internship) {
        ...
        mailOptions = {
            ...
            &#8226; <strong> Tour comments: </strong> ${insertResult.rows[0].comments} <br>
            ${additionalText}<br> If they requested any additional packages, you will have to reach out manually and 
            ...
            &#8226; <strong> Packages: </strong>Again, if any additional packages are requested, you will have to reach
            out and coordinate the additional logistics with them.<br><br><br>
...
exports.deleteTour = async (req, res) => {
    try {
        ...
        const tourQuery = "SELECT tour_guide_id, client_id, event_date, greek_life, \
                            student_athlete, paid_internship FROM tours WHERE id = $1";
        const tourResult = await db.query(tourQuery, [id]);
        if (tourResult.rows.length === 0) {
            res.status(404).json({ message: 'Tour not found' });
            return;
        ...
        let additionalText = "";
        if (tourResult.rows[0].greek_life) {
            additionalText += "<br><br>The client also opted into the <strong> greek life </strong> package for an extra $50.";
        }
        if (tourResult.rows[0].student_athlete) {
            additionalText += "<br><br>The client also opted into the <strong> student athlete </strong>package for an extra $50.";
        }
        if (tourResult.rows[0].paid_internship) {
            additionalText += "<br><br>The client also opted into the <strong> paid internship chat </strong> package for an extra $40.";
        }
        // Send email to Josh
        mailOptions = {
            from: process.env.EMAIL_USERNAME,
