...
const generateZoomVideoSDKToken = (eventDate) => {
    // Split the 'MM/DD/YYYY' formatted date into components
    const [month, day, year] = eventDate.split('/').map(Number);

    // Create a JavaScript Date object from the components (month is 0-indexed in JavaScript)
    ...
    // Add 7 days (one week) to the event date to set the expiration
    const expirationDate = new Date(eventDateObj);
    expirationDate.setDate(expirationDate.getDate() + 7);

    // Convert expirationDate to a UNIX timestamp (seconds since epoch)
    const expirationTimestamp = Math.floor(expirationDate.getTime() / 1000); // In seconds

    // Prepare the payload for the JWT token
    const payload = {
        app_key: process.env.ZOOM_SDK_KEY, // Your Zoom SDK Key
        version: 1,                        // Fixed value for SDK
        tpc: "TouredIt_Video_Session",     // Unique session name/topic
        // Set the expiration to one week after the event date
        exp: expirationTimestamp,
    };

    // Sign the token with your SDK Secret
...
const createZoomVideoSession = async (date) => {
    try {
        const token = generateZoomVideoSDKToken(date); // JWT Token generated
...
exports.confirmGuideCall = async (req, res) => {
    try {
        ...
        const videoSession = await createZoomVideoSession(callResult.rows[0].eventDate);
        const emailLink = `https://TouredIt.com/video/${videoSession.sessionName}?code=${videoSession.token}`;
        ...
        mailOptions = {
            ...
            &#8226; TouredIt Video Call Link: <a href="${const emailLink = `https://yourdomain.com/video/${videoSession.sessionName}?token=${videoSession.token}`;
            }">${videoSession}</a><br><br> 
            <br><br>
        ...
        mailOptions = {
            ...
            &#8226; Zoom Meet Link: <a href="${videoSession}">${videoSession}</a><br><br> 
            \t &#8226; <strong> The guide details </strong><br>\t\t
            &#8226; <strong> Guide name:</strong> ${guideResult.rows[0].name}<br>\t\t
            &#8226; <strong> Guide email:</strong> ${guideResult.rows[0].email}<br>\t\t
            &#8226; <strong> Guide School: </strong> ${guideResult.rows[0].school} <br><br>
