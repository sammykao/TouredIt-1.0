...
exports.addHobbies = async (req, res) => {
    try {
        ...
        const updateOrInsertQuery = `
            ...
            BEGIN
                IF EXISTS (
                    ...
                    WHERE tourguide_id = (SELECT id FROM tour_guides WHERE LOWER(email) = LOWER($1))
                    AND hobby_name = $2
                ) THEN
                    UPDATE hobbies
                    SET description = $3
                ...
                ELSE
                    ...
                    VALUES ((SELECT id FROM tour_guides WHERE LOWER(email) = LOWER($1)), $2, $3);
                END IF;
            END
            $$;
        `;
        ...
        await db.query(updateOrInsertQuery, values);
...
exports.addInvolvement = async (req, res) => {
    try {
        ...
        const updateOrInsertQuery = `
            ...
            BEGIN
                IF EXISTS (
                    SELECT 1 FROM campus_involvement
                    WHERE tourguide_id = (SELECT id FROM tour_guides WHERE LOWER(email) = LOWER($1))
                    AND activity_name = $2
                ) THEN
                    UPDATE campus_involvement
                    SET description = $3
                    WHERE tourguide_id = (SELECT id FROM tour_guides WHERE LOWER(email) = LOWER($1))
                    AND activity_name = $2;
                ELSE
