...
exports.confirmGuideCall = async (req, res) => {
    try {
        ...
        let mailOptions = {
            ...
            and will reach out about specific scheduling logistics, as well as sending you the zoom link. 
        ...
        await sendEmailWithRetry(mailOptions);
        const zoomMeeting = await createZoomMeeting(guideResult.rows[0].email, callResult.rows[0].event_date);
        // Send email to the guide
        mailOptions = {
            from: process.env.EMAIL_USERNAME,
            to: guideResult.rows[0].email,
...
async function createZoomMeeting(guideEmail, eventDate) {
    try {
        const response = await axios.post(
            'https://api.zoom.us/v2/users/me/meetings',
            {
                topic: 'Video Chat with Client',
                type: 2, // Scheduled meeting
                start_time: eventDate, // Event date from the call
            ...
            {
                headers: {
                    'Authorization': `Bearer ${process.env.ZOOM_JWT_TOKEN}`, // Use your Zoom JWT Token
        ...
        throw new Error(`Failed to create Zoom meeting: ${error.response ? error.response.data.message : error.message}`);
    }
}
