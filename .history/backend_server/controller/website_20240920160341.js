...
exports.insertNewsletter = async (req, res) => {
    try {
        ...
        if (!email || api_key !== "admin_control_345dc**nIIlpa") {
            res.status(400).json({ message: 'Invalid client request' });
            return;
        }

        // Check if the email already exists in the newsletter table
        const checkQuery = "SELECT email FROM newsletter WHERE email = $1";
        ...
        const insertQuery = "INSERT INTO newsletter (email) VALUES ($1) RETURNING *";
        const insertResult = await db.query(insertQuery, [email]);

        res.status(201).json({ message: 'Email successfully added to newsletter', data: insertResult.rows[0] });
