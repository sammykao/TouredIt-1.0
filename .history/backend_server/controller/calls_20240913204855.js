...
// Helper function to parse 'MM/DD/YYYY' format and generate a valid Date object
...
// Helper function to generate a JWT token for Vonage Video with expiration based on event date
const generateVonageVideoToken = (sessionId, eventDate) => {
    // Parse event date and convert to JavaScript Date object
    const eventDateObj = parseEventDate(eventDate);

    // Add 7 days (one week) to the event date to calculate the expiration
    const expirationDate = new Date(eventDateObj);
    expirationDate.setDate(expirationDate.getDate() + 7);

    // Convert expirationDate to a UNIX timestamp in seconds
...
const createVonageVideoCall = async (event_date) => {
    try {
        const session = await createVonageVideoSession();
        const token = generateVonageVideoToken(session.sessionId, event_date);
...
exports.confirmGuideCall = async (req, res) => {
    try {
        ...
        const videoSession = await createVonageVideoCall(callResult.rows[0].event_date);
