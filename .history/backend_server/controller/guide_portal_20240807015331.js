...
// Helper function to validate account data
...
// Function to insert guide accounts
...
exports.addHobbies = async (req, res) => {
    try {
        ...
        if (!email || !hobby_name || !description) {
            res.status(400).send(JSON.stringify({ message: 'Missing required hobby data' }));
            return;
        }
        ...
        const result = await db.query(query, values);
        res.status(201).send(JSON.stringify({ message: 'Hobby added', hobby: result.rows[0] }));
...
exports.addInvolvement = async (req, res) => {
    try {
        ...
        const result = await db.query(query, values);
        res.status(201).send(JSON.stringify({ message: 'Involvement added', involvement: result.rows[0] }));
...
exports.deleteHobby = async (req, res) => {
    try {
        ...
        if (!hobby_id) {
            res.status(400).send(JSON.stringify({ message: 'Missing hobby ID' }));
            return;
        }
        ...
        if (result.rows.length === 0) {
            res.status(404).send(JSON.stringify({ message: 'Hobby not found' }));
        } else {
            res.status(200).send(JSON.stringify({ message: 'Hobby deleted', hobby: result.rows[0] }));
        }
...
// Function to delete an involvement
exports.deleteInvolvement = async (req, res) => {
    try {
        const { id } = req.body;

        if (!id) {
            res.status(400).send(JSON.stringify({ message: 'Missing involvement ID' }));
            return;
        }
        ...
        if (result.rows.length === 0) {
            res.status(404).send(JSON.stringify({ message: 'Involvement not found' }));
        } else {
            res.status(200).send(JSON.stringify({ message: 'Involvement deleted', involvement: result.rows[0] }));
        }
...
exports.retrieveHobbies = async (req, res) => {
    try {
        ...
        if (!tourguide_id) {
            res.status(400).send(JSON.stringify({ message: 'Missing tour guide ID' }));
            return;
        }
        ...
        if (result.rows.length === 0) {
            res.status(404).send(JSON.stringify({ message: 'No hobbies found' }));
        } else {
            res.status(200).send(JSON.stringify({ hobbies: result.rows }));
        }
...
// Function to retrieve involvement
exports.retrieveInvolvement = async (req, res) => {
    try {
        ...
        if (result.rows.length === 0) {
            res.status(404).send(JSON.stringify({ message: 'No involvements found' }));
        } else {
            res.status(200).send(JSON.stringify({ involvements: result.rows }));
        }
...
exports.retrieveTours = async (req, res) => {
    try {
        ...
        if (!email) {
            res.status(400).send(JSON.stringify({ message: 'Invalid guide email' }));
            return;
        }
...
// Function to update guide accounts
...
// Function to retrieve guide info
...
// Function to confirm a tour
exports.confirmTour = async (req, res) => {
    try {
        const { id } = req.body;
        if (!id) {
            res.status(400).send(JSON.stringify({ message: 'Invalid request: Missing required fields' }));
            return;
        }
        ...
        if (tourResult.rows.length === 0) {
            res.status(404).send(JSON.stringify({ message: 'Tour not found' }));
            return;
        }
...
exports.deleteTour = async (req, res) => {
    try {
        ...
        res.status(200).send(JSON.stringify({ message: 'request submitted successfully', tour: deleteResult.rows[0] }));
