    ...
    try {
        ...
        const query = `
            SELECT 
                ...
                COALESCE(array_agg(DISTINCT ca.activity_name) FILTER (WHERE ca.activity_name IS NOT NULL), ARRAY[]::VARCHAR[]) AS activities,
COALESCE(array_agg(DISTINCT h.hobby_name) FILTER (WHERE h.hobby_name IS NOT NULL), ARRAY[]::VARCHAR[]) AS hobbies
            FROM 
                tour_guides tg
            LEFT JOIN 
