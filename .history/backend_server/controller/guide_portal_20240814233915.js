...
exports.retrieveGuideInfo = async (req, res) => {
    try {
        ...
        const query = `
            SELECT 
                json_build_object(
                    ...
                    'email', tg.email,
                    'school', tg.school,
                    'hometown', tg.hometown,
                    'phone', tg.phone,
                    'bio', tg.bio,
                    'major', tg.major,
                    'secondary_major', tg.secondary_major,
                    'minor', tg.minor,
                    'secondary_minor', tg.secondary_minor,
                    'profile_image_url', tg.profile_image_url,
                    'instagram', tg.instagram,
                    'linkedin', tg.linkedin,
                    'num_tours', tg.num_tours,
                    'greek_life', tg.greek_life,
                    'student_athlete', tg.student_athlete,
                    'paid_internship', tg.paid_internship,
                    'activities', COALESCE(json_agg(DISTINCT json_build_object(
                        'activity_name', ca.activity_name,
                        'description', ca.description,
                        'activity_id', ca.id
                    )) FILTER (WHERE ca.activity_name IS NOT NULL), '[]'),
                    'hobbies', COALESCE(json_agg(DISTINCT json_build_object(
                        'hobby_name', h.hobby_name,
                        'description', h.description,
                        'hobby_id', h.hobby_id
                    )) FILTER (WHERE h.hobby_name IS NOT NULL), '[]'),
                    'work_experience', COALESCE(json_agg(DISTINCT json_build_object(
                        'job_name', w.job_name,
                        'description', w.description,
                        'job_id', w.id
                    )) FILTER (WHERE w.job_name IS NOT NULL), '[]')
                ) AS guide
            FROM 
                tour_guides tg
            LEFT JOIN 
            ...
            LEFT JOIN 
                work_experience w ON tg.id = w.tourguide_id
            WHERE 
                LOWER(tg.email) = LOWER($1)
            GROUP BY 
                tg.id`;
        ...
        return res.status(200).json({guide: result.rows[0]});
